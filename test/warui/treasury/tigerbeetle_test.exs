defmodule Warui.Treasury.TigerbeetleTest do
  ...
  describe "account operations" do
    test "create_account/1 creates an account" do
      ...
      assert {:ok, tb_account} =
               TigerbeetleService.create_account(attrs, user, organization_owner)
    ...
    test "get_account_balance/1 returns the correct balance" do
      ...
      assert {:ok, query_result} =
               TigerbeetleService.query_accounts(filter, user1, organization_owner1)
      ...
      key =
        {:ledger_asset_type, :id, {organization_owner3.current_organization, transfer_ledger.id}}
  ...
  describe "transfer operations" do
    test "create_transfer/1 creates a transfer between accounts" do
      ...
      assert {:ok, query_result} =
               TigerbeetleService.query_transfers(filter, user1, organization_owner3)
      ...
      assert {:ok, query_result} =
               TigerbeetleService.query_transfers(filter, user1, organization_owner1)
    ...
    test "get_account_transfers/2 retrieves transfers for an account" do
      ...
      assert {:ok, account1_transfers} =
               TigerbeetleService.get_account_transfers(filter, user1, organization_owner1)
      ...
      assert {:ok, account2_transfers} =
               TigerbeetleService.get_account_transfers(filter, user2, organization_owner3)
